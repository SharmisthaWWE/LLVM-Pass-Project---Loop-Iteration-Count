#ifndef _LOOP_ITERATION_COUNTER
#define _LOOP_ITERATION_COUTNER
#include "llvm/Pass.h"
...
namespace {
	...
	static Function* printHelper(LLVMContext& theContext, Module *newModule, Constant* format)
	{
		...
		// New version uses AttributeList instead of Set.
		Function* Func = cast<Function>(newModule->getOrInsertFunction("printf", printType, AttributeList().addAttribute(newModule->getContext(), 1U, Attribute::NoAlias)));
		...
		virtual bool runOnFunction(Function&F) {
			...
			for (LoopInfo::iterator i = LI.begin(), e = LI.end(); i != e; ++i)
			{
				...
				for (j = subLoopAll.begin(), f = subLoopAll.end(); j != f; ++j)
				{
					...
					Instruction *ACounter = new AllocaInst(Type::getInt32Ty(mContext), 0, IntVal4, "counter");  //(IntegerType::get(mContext, 32), one, "counter", loopPredecessor);  //(IntegerType::get(mContext, 32), IR.getInt32(0), "counter");
